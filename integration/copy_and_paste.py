...
def integration_copy_paste(pred_body_list, pred_hand_list, smplx_model, image_shape):
    ...
    for i in range(len(pred_body_list)):
        ...
        pred_aa_tensor = gu.rotation_matrix_to_angle_axis(pred_rotmat.detach()[0])
        integral_output['pred_body_pose'] = pred_aa_tensor.numpy().reshape(1, 72)
        integral_output['pred_betas'] = pred_betas.detach().numpy()
        ...
        r_hand_global_orient_body = transfer_rotation(
            ...
            torch.from_numpy(r_hand_local_orient_body),
        ...
        l_hand_global_orient_body = transfer_rotation(
            smplx_model, pred_rotmat,
            torch.from_numpy(l_hand_local_orient_body),
        ...
        if right_hand_local_orient is not None:
            r_hand_local_orient_hand = gu.rotation_matrix_to_angle_axis(
                right_hand_local_orient).detach().numpy().reshape(1, 3)
        ...
        if left_hand_local_orient is not None:
            l_hand_local_orient_hand = gu.rotation_matrix_to_angle_axis(
                left_hand_local_orient).detach().numpy().reshape(1, 3)
        ...
        integral_output['pred_left_hand_pose'] = left_hand_pose.detach().numpy()
        integral_output['pred_right_hand_pose'] = right_hand_pose.detach().numpy()
